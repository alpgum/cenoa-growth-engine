#!/usr/bin/env python3
"""
Amplitude Country Breakdown Script
Pulls core funnel events segmented by country from Amplitude's Segmentation API.

Usage:
    python amplitude_country_breakdown.py [--start YYYYMMDD] [--end YYYYMMDD]
...
def fetch_event_by_country(api_key, secret_key, event_type, start, end):
    """Fetch a single event segmented by country."""
    e_param = json.dumps({
...
def parse_country_data(response_data):
    """Parse Amplitude segmentation response into country breakdown."""
    ...
    for i, label_list in enumerate(series_labels):
        # label_list is like ["event_type", "country_name"]
        country = label_list[1] if len(label_list) > 1 else "Unknown"
        ...
        if value > 0:
            country_counts[country] = country_counts.get(country, 0) + value

    return country_counts


def add_percentages(country_counts):
    """Add percentage splits to country data."""
...
def main():
    parser = argparse.ArgumentParser(description="Amplitude country breakdown")
    ...
    print(f"📊 Amplitude Country Breakdown: {args.start} → {args.end}")
    print("=" * 60)

    results = {}
    for event in FUNNEL_EVENTS:
        print(f"\n🔍 Fetching: {event}")
        resp = fetch_event_by_country(api_key, secret_key, event, args.start, args.end)
        country_counts = parse_country_data(resp)
        breakdown = add_percentages(country_counts)
        ...
        for entry in breakdown[:10]:
            print(f"  {entry['country']:>20s}: {entry['count']:>6,} ({entry['percentage']:>5.1f}%)")
        if len(breakdown) > 10:
            print(f"  ... and {len(breakdown) - 10} more countries")
    ...
    output_path = data_dir / f"amplitude-country-{end_date_fmt}.json"
